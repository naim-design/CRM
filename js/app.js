...
function updateTopupVisibility() {
  ...
  const susuCrossSell = document.getElementById('kategori-susu-cross-sell-flow');
  if (susuCrossSell) {
    const showSusuCrossSell = kategori === 'Projek Susu';
    susuCrossSell.style.display = showSusuCrossSell ? 'block' : 'none';
    if (showSusuCrossSell) {
      setTimeout(() => resizeSusuCrossSellFrame(document.getElementById('projek-susu-cross-sell-frame')), 120);
    }
  }
...
// ---- Projek Susu cross-sell presentation embedded inside Dashboard ----
function resizeSusuCrossSellFrame(frame) {
  if (!frame) return;
  try {
  ...
  wrap.innerHTML=rows.map(r=>{
    ...
    return `<article class="seg-saved-card ${isStopSave?'stop-save':''}">
      ...
      <div class="seg-saved-actions">
        <button class="btn btn-primary seg-use-saved-filter" data-id="${r.id}">↻ Guna Filter</button>
...
document.getElementById('seg-save-result-btn')?.addEventListener('click',async()=>{
  // V44: Simpan filter walaupun belum ada result.
  // Kalau ada result semasa, nombor itu disimpan sekali.
  // Kalau user taip nombor tetapi nombor tiada DB, nombor itu tetap disimpan sebagai unmatched.
  ...
  const defaultName =
    phone ? `Nombor ${phone}` :
    status==='buyer' ? 'Buyer Retarget' :
    status==='replied' ? 'Reply Retarget' :
    source ? `Filter ${source}` :
    'Filter Retarget';

  const name=prompt('Nama simpanan filter ini:', defaultName);
  if(name===null) return;
  ...
  if(phone && !allPhones.some(p=>segNormalizePhone(p)===segNormalizePhone(phone))){
    unmatchedPhones=segUniquePhones([...unmatchedPhones,phone]);
    addPersistentUnmatched([phone]);
  }

  try{
    await db.collection('filterSaves').add({
      name:(name||'Filter Retarget').trim(),
      total:summary.total||allPhones.length||0,
      buyer:summary.buyer||buyerPhones.length||0,
      replied:summary.replied||replyPhones.length||0,
...
'YGROW':{total:3,video:1,photoslide:1,thread:1,breakdown:{Video:{video:1,photoslide:0},Thread:{video:0,photoslide:0},Poster:{video:0,photoslide:1}}}};
const CREATIVE_PLATFORM_META={Website:'website',TikTok:'tiktok',WhatsApp:'whatsapp',Shopee:'shopee',Ecommerce:'ecommerce',Packaging:'packaging','Sosial Media':'socialmedia',Photoshoot:'photoshoot'};
let creativeRows=[],creativeListenerStarted=false,creativeActiveTab='video',creativeEditingId=null,creativePosterImageData='',creativePosterImages=[],creativeFlatlayRows=[],creativePlanRows=[],creativeFlatlayListenerStarted=false,creativePlanListenerStarted=false,creativePlanEditingId=null,creativeFlatlayEditingId=null;
...
function renderCreativeVideo(){const rows=cRows('video'),days=cDays(),target=35*days,posted=rows.filter(r=>r.status==='posted').length,pct=target?Math.min(100,Math.round(posted/target*100)):0;cSet('creative-target-all',target);cSet('creative-posted-all',posted);cSet('creative-remaining-all',Math.max(0,target-posted));cSet('creative-progress-all',pct+'%');cSet('creative-target-period',`${days} hari × 35 content`);cBar('creative-progress-bar',pct);
const g=document.getElementById('creative-account-grid');if(g)g.innerHTML=Object.entries(CREATIVE_ACCOUNT_TARGETS).map(([n,t])=>{const ar=rows.filter(r=>r.account===n),ap=ar.filter(r=>r.status==='posted'),at=t.total*days,p=at?Math.min(100,Math.round(ap.length/at*100)):0,vd=ap.filter(r=>r.format==='video').length,ph=ap.filter(r=>r.format==='photoslide').length,th=ap.filter(r=>r.format==='thread').length;let bd='';if(n==='YGROW'){bd=`<li><span>Video</span><b>${vd}/${days}</b></li><li><span>Post Thread</span><b>${th}/${days}</b></li><li><span>Poster</span><b>${ph}/${days}</b></li>`;}else{bd=Object.entries(t.breakdown).map(([f,b])=>{const v=ap.filter(r=>r.funnel===f&&r.format==='video').length,q=ap.filter(r=>r.funnel===f&&r.format==='photoslide').length;return `<li><span>${f}</span><b>${v}V${b.photoslide?` + ${q}P`:''} / ${b.video*days}V${b.photoslide?` + ${b.photoslide*days}P`:''}</b></li>`;}).join('');}const mix=n==='YGROW'?`Video ${vd}/${days} · Thread ${th}/${days} · Poster ${ph}/${days}`:`Video ${vd}/${t.video*days} · Photo Slide ${ph}/${t.photoslide*days}`;const accountCls=n==="HQ"?"acc-hq":n==="Solusi"?"acc-solusi":n==="Mamayyuu"?"acc-mamayyuu":n==="Manjaratu V+"?"acc-manjaratu":"acc-ygrow";return `<article class="creative-account-card ${accountCls}"><div class="creative-account-card-top"><div><b>${n}</b><span>${ap.length}/${at} posted</span></div><strong>${p}%</strong></div><div class="creative-progress"><i style="width:${p}%"></i></div><small>${mix}</small><ul>${bd}</ul></article>`;}).join('');
...
async function cLoadPosterMedia(posterId){
  if(!posterId)return [];
  try{
    const s=await db.collection('creativePosterMedia').where('posterId','==',posterId).get();
    return s.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>(a.order||0)-(b.order||0)).map(x=>x.dataUrl).filter(Boolean);
  }catch(err){console.warn('Poster media load:',err);return [];}
}
async function cDeletePosterMedia(posterId){
  const s=await db.collection('creativePosterMedia').where('posterId','==',posterId).get();
  const chunks=[]; let batch=db.batch(),n=0;
  for(const d of s.docs){batch.delete(d.ref);n++;if(n===400){chunks.push(batch.commit());batch=db.batch();n=0;}}
  if(n)chunks.push(batch.commit()); await Promise.all(chunks);
}
async function cSavePosterMedia(posterId,files){
  if(!posterId||!files?.length)return;
  await cDeletePosterMedia(posterId);
  const chunks=[]; let batch=db.batch(),n=0;
  files.forEach((dataUrl,i)=>{
    const ref=db.collection('creativePosterMedia').doc();
    batch.set(ref,{posterId,order:i,dataUrl,createdAt:firebase.firestore.FieldValue.serverTimestamp()});
    n++; if(n===400){chunks.push(batch.commit());batch=db.batch();n=0;}
  });
  if(n)chunks.push(batch.commit()); await Promise.all(chunks);
}
function cOpen(type,id=null){creativeEditingId=id;const r=id?creativeRows.find(x=>x.id===id):null;if(type==='video'){document.getElementById('creative-video-form-title').textContent=r?'Edit Content':'Tambah Content';document.getElementById('cv-date').value=r?.date||cToday();document.getElementById('cv-account').value=r?.account||'HQ';document.getElementById('cv-format').value=r?.format||'video';document.getElementById('cv-funnel').value=r?.funnel||'Engagement';document.getElementById('cv-status').value=r?.status||'belum';document.getElementById('cv-link').value=r?.tiktokLink||'';document.getElementById('cv-title').value=r?.title||'';[['24','h24'],['3','d3'],['7','d7'],['30','d30']].forEach(([p,k])=>['view','like','comment'].forEach(m=>document.getElementById(`cv-${p}-${m}`).value=r?.insights?.[k]?.[m]||0));document.getElementById('creative-video-modal').classList.add('open');}else{creativePosterImageData=r?.mediaData||'';creativePosterImages=Array.isArray(r?.mediaFiles)?[...r.mediaFiles]:(creativePosterImageData?[creativePosterImageData]:[]);
if(r?.id&&r?.mediaCount){cLoadPosterMedia(r.id).then(arr=>{if(arr.length){creativePosterImages=arr;creativePosterImageData=arr[0]||'';cPreview();}});}document.getElementById('creative-poster-form-title').textContent=r?'Edit Poster':'Tambah Poster';document.getElementById('cp-date').value=r?.date||cToday();document.getElementById('cp-platform').value=r?.platform||'Website';document.getElementById('cp-status').value=r?.status||'belum';document.getElementById('cp-title').value=r?.title||'';document.getElementById('cp-note').value=r?.note||'';cPreview();document.getElementById('creative-poster-modal').classList.add('open');}}
function cClose(id){document.getElementById(id)?.classList.remove('open');creativeEditingId=null;}
function cPreview(){const w=document.getElementById('cp-preview-wrap');if(!w)return;const files=creativePosterImages.length?creativePosterImages:(creativePosterImageData?[creativePosterImageData]:[]);w.innerHTML=files.length?files.map((src,i)=>`<button type="button" class="creative-gallery-item" data-gallery-index="${i}"><img src="${src}" alt="media ${i+1}"><span>${i+1}</span></button>`).join(''):'<span>Belum ada media</span>';}
function cCompress(f){return new Promise((res,rej)=>{const fr=new FileReader();fr.onerror=()=>rej(new Error('Gagal baca gambar'));fr.onload=()=>{const im=new Image();im.onload=()=>{let w=im.width,h=im.height,sc=Math.min(1,1000/w,1000/h);w=Math.round(w*sc);h=Math.round(h*sc);const c=document.createElement('canvas');c.width=w;c.height=h;c.getContext('2d').drawImage(im,0,0,w,h);let out=c.toDataURL('image/jpeg',.72);if(out.length>850000)out=c.toDataURL('image/jpeg',.58);res(out);};im.onerror=()=>rej(new Error('Format gambar tak disokong'));im.src=fr.result;};fr.readAsDataURL(f);});}
document.querySelectorAll('[data-creative-tab]').forEach(b=>b.addEventListener('click',()=>{creativeActiveTab=b.dataset.creativeTab;document.querySelectorAll('[data-creative-tab]').forEach(x=>x.classList.toggle('active',x===b));document.querySelectorAll('.creative-tab-pane').forEach(p=>p.classList.toggle('active',p.id===`creative-tab-${creativeActiveTab}`));if(creativeActiveTab==='planning'){startCreativePlanListener();renderCreativePlanning();}if(creativeActiveTab==='poster')startCreativeFlatlayListener();}));
document.querySelectorAll('[data-crange]').forEach(b=>b.addEventListener('click',()=>setCreativeRange(b.dataset.crange)));document.getElementById('creative-from')?.addEventListener('change',renderCreativeControl);document.getElementById('creative-to')?.addEventListener('change',renderCreativeControl);document.getElementById('creative-add-btn')?.addEventListener('click',()=>{if(creativeActiveTab==='poster')cOpen('poster');else if(creativeActiveTab==='planning')openPlan();else cOpen('video');});document.getElementById('creative-add-video')?.addEventListener('click',()=>cOpen('video'));document.getElementById('creative-add-poster')?.addEventListener('click',()=>cOpen('poster'));document.querySelectorAll('[data-close-modal]').forEach(b=>b.addEventListener('click',()=>cClose(b.dataset.closeModal)));
document.getElementById('cp-media')?.addEventListener('change',async e=>{const files=[...(e.target.files||[])];if(!files.length)return;if(files.length>8){toast('Maksimum 8 gambar untuk satu rekod poster.',true);return;}try{creativePosterImages=[];for(const f of files)creativePosterImages.push(await cCompress(f));creativePosterImageData=creativePosterImages[0]||'';cPreview();}catch(err){toast(err.message,true);}});
...
document.getElementById('creative-poster-form')?.addEventListener('submit',async e=>{
  e.preventDefault();
  const hasSelected=creativePosterImages.length>0;
  const d={kind:'poster',date:document.getElementById('cp-date').value,platform:document.getElementById('cp-platform').value,status:document.getElementById('cp-status').value,title:document.getElementById('cp-title').value.trim(),note:document.getElementById('cp-note').value.trim(),updatedAt:firebase.firestore.FieldValue.serverTimestamp()};
  try{
    let posterId=creativeEditingId;
    if(posterId){
      if(hasSelected){
        d.mediaCount=creativePosterImages.length;
        d.mediaData=firebase.firestore.FieldValue.delete();
        d.mediaFiles=firebase.firestore.FieldValue.delete();
      }
      await db.collection('creativeTracker').doc(posterId).update(d);
    }else{
      d.createdAt=firebase.firestore.FieldValue.serverTimestamp();
      d.mediaCount=creativePosterImages.length;
      const ref=await db.collection('creativeTracker').add(d);posterId=ref.id;
    }
    if(hasSelected)await cSavePosterMedia(posterId,creativePosterImages);
    toast(`Poster disimpan ✓${creativePosterImages.length?` (${creativePosterImages.length} file)`:''}`);
    cClose('creative-poster-modal');creativePosterImageData='';creativePosterImages=[];
  }catch(err){toast('Gagal simpan poster: '+err.message,true);}
});
...
document.getElementById('creative-poster-body')?.addEventListener('click',async e=>{const ed=e.target.closest('[data-pedit]'),del=e.target.closest('[data-cdelete]'),files=e.target.closest('[data-files]');if(ed)cOpen('poster',ed.dataset.pedit);if(files){const r=creativeRows.find(x=>x.id===files.dataset.files);let arr=r?.mediaCount?await cLoadPosterMedia(r.id):[];if(!arr.length)arr=r?.mediaFiles?.length?r.mediaFiles:(r?.mediaData?[r.mediaData]:[]);if(arr.length){const ov=document.createElement('div');ov.className='creative-lightbox';ov.innerHTML=`<div class="creative-lightbox-card"><button class="creative-lightbox-close">×</button><div class="creative-lightbox-grid">${arr.map((s,i)=>`<a href="${s}" target="_blank"><img src="${s}" alt="file ${i+1}"></a>`).join('')}</div></div>`;document.body.appendChild(ov);ov.addEventListener('click',x=>{if(x.target===ov||x.target.closest('.creative-lightbox-close'))ov.remove();});}}if(del&&confirm('Padam rekod ini?')){await cDeletePosterMedia(del.dataset.cdelete);await db.collection('creativeTracker').doc(del.dataset.cdelete).delete();}});


// ================= V50 FLATLAY + CONTENT PLANNING =================
function startCreativeFlatlayListener(){
  if(creativeFlatlayListenerStarted)return; creativeFlatlayListenerStarted=true;
  db.collection('creativeFlatlays').onSnapshot(s=>{creativeFlatlayRows=s.docs.map(d=>({id:d.id,...d.data()}));renderCreativeFlatlays();},e=>toast('Ralat baca Flatlay: '+e.message,true));
}
function renderCreativeFlatlays(){
  const b=document.getElementById('creative-flatlay-body');if(!b)return;
  const f=document.getElementById('creative-from')?.value||'',t=document.getElementById('creative-to')?.value||'';
  const rows=creativeFlatlayRows.filter(r=>(!f||r.date>=f)&&(!t||r.date<=t)).sort((a,b)=>(b.date||'').localeCompare(a.date||''));
  b.innerHTML=rows.length?rows.map(r=>`<tr><td>${r.date||'-'}</td><td><b>${cEsc(r.title||'-')}</b></td><td>${cEsc(r.category||'-')}</td><td>${r.driveLink?`<a class="creative-drive-btn" href="${cEsc(r.driveLink)}" target="_blank" rel="noopener">Buka Google Drive ↗</a>`:'–'}</td><td>${cEsc(r.note||'-')}</td><td><button class="creative-action edit" data-fedit="${r.id}">Edit</button><button class="creative-action delete" data-fdelete="${r.id}">Padam</button></td></tr>`).join(''):'<tr><td colspan="6" class="empty-state">Belum ada flatlay dalam julat tarikh ini.</td></tr>';
}
function openFlatlay(id=null){
  creativeFlatlayEditingId=id;const r=id?creativeFlatlayRows.find(x=>x.id===id):null;
  document.getElementById('creative-flatlay-form-title').textContent=r?'Edit Flatlay':'Tambah Flatlay';
  document.getElementById('cf-date').value=r?.date||cToday();document.getElementById('cf-title').value=r?.title||'';document.getElementById('cf-category').value=r?.category||'';document.getElementById('cf-link').value=r?.driveLink||'';document.getElementById('cf-note').value=r?.note||'';
  document.getElementById('creative-flatlay-modal').classList.add('open');
}
document.getElementById('creative-add-flatlay')?.addEventListener('click',()=>openFlatlay());
document.getElementById('creative-flatlay-form')?.addEventListener('submit',async e=>{e.preventDefault();const d={date:document.getElementById('cf-date').value,title:document.getElementById('cf-title').value.trim(),category:document.getElementById('cf-category').value.trim(),driveLink:document.getElementById('cf-link').value.trim(),note:document.getElementById('cf-note').value.trim(),updatedAt:firebase.firestore.FieldValue.serverTimestamp()};try{if(creativeFlatlayEditingId)await db.collection('creativeFlatlays').doc(creativeFlatlayEditingId).update(d);else{d.createdAt=firebase.firestore.FieldValue.serverTimestamp();await db.collection('creativeFlatlays').add(d);}toast('Flatlay disimpan ✓');document.getElementById('creative-flatlay-modal').classList.remove('open');creativeFlatlayEditingId=null;}catch(err){toast('Gagal simpan flatlay: '+err.message,true);}});
document.getElementById('creative-flatlay-body')?.addEventListener('click',async e=>{const ed=e.target.closest('[data-fedit]'),del=e.target.closest('[data-fdelete]');if(ed)openFlatlay(ed.dataset.fedit);if(del&&confirm('Padam flatlay ini?'))await db.collection('creativeFlatlays').doc(del.dataset.fdelete).delete();});

function startCreativePlanListener(){
  if(creativePlanListenerStarted)return;creativePlanListenerStarted=true;
  db.collection('creativePlans').onSnapshot(s=>{creativePlanRows=s.docs.map(d=>({id:d.id,...d.data()}));renderCreativePlanning();},e=>toast('Ralat baca Content Planning: '+e.message,true));
}
function promoForDate(iso){
  if(!iso||!iso.startsWith('2026-09-'))return null;
  const d=Number(iso.slice(8,10));
  if(d<=9)return {name:'Promo Double Digit',cls:'promo-double'};
  if(d<=21)return {name:'Promo September Lebih Jimat',cls:'promo-jimat'};
  return {name:'Promo Payday',cls:'promo-payday'};
}
function renderCreativePlanning(){
  const month=document.getElementById('cplan-month')?.value||'2026-09';
  const [y,m]=month.split('-').map(Number),first=new Date(y,m-1,1),lastDay=new Date(y,m,0).getDate();
  const offset=(first.getDay()+6)%7;
  const grid=document.getElementById('creative-calendar-grid');
  if(grid){
    let out='';for(let i=0;i<offset;i++)out+='<div class="creative-cal-day blank"></div>';
    for(let d=1;d<=lastDay;d++){
      const iso=`${y}-${String(m).padStart(2,'0')}-${String(d).padStart(2,'0')}`,p=promoForDate(iso),plans=creativePlanRows.filter(r=>r.date===iso);
      const done=plans.filter(r=>r.status==='done').length,progress=plans.filter(r=>r.status==='progress').length,belum=plans.filter(r=>r.status==='belum').length;
      out+=`<div class="creative-cal-day ${p?.cls||''}" data-cal-date="${iso}"><div class="creative-cal-date"><b>${d}</b><span>35 content</span></div>${p?`<div class="creative-cal-promo">${p.name}</div>`:''}<div class="creative-cal-target">HQ 11 · Solusi 11 · Mamayyuu 9 · Manjaratu 1 · YGROW 3</div><div class="creative-cal-plans">${plans.slice(0,3).map(r=>`<span class="${r.status}">${cEsc(r.title)}</span>`).join('')}${plans.length>3?`<small>+${plans.length-3} lagi</small>`:''}</div><div class="creative-cal-status"><i class="done"></i>${done}<i class="progress"></i>${progress}<i class="belum"></i>${belum}</div></div>`;
    }grid.innerHTML=out;
  }
  const body=document.getElementById('creative-plan-body');if(body){
    const rows=creativePlanRows.filter(r=>(r.date||'').startsWith(month)).sort((a,b)=>(a.date||'').localeCompare(b.date||''));
    body.innerHTML=rows.length?rows.map(r=>`<tr><td>${r.date||'-'}</td><td><span class="creative-role">${cEsc(r.role||'-')}</span></td><td>${cEsc(r.account||'-')}</td><td>${cEsc(r.type||'-')}</td><td><b>${cEsc(r.title||'-')}</b></td><td><span class="creative-status ${r.status}">${r.status==='done'?'Done':r.status==='progress'?'In Progress':'Belum'}</span></td><td>${r.resultLink?`<a class="creative-drive-btn" href="${cEsc(r.resultLink)}" target="_blank" rel="noopener">Buka Link ↗</a>`:'–'}</td><td>${cEsc(r.note||'-')}</td><td><button class="creative-action edit" data-pledit="${r.id}">Edit</button><button class="creative-action delete" data-pldelete="${r.id}">Padam</button></td></tr>`).join(''):'<tr><td colspan="9" class="empty-state">Belum ada planning untuk bulan ini.</td></tr>';
  }
}
function openPlan(id=null,date=null){
  creativePlanEditingId=id;const r=id?creativePlanRows.find(x=>x.id===id):null;
  document.getElementById('creative-plan-form-title').textContent=r?'Edit Planning':'Tambah Planning';
  document.getElementById('cpl-date').value=r?.date||date||cToday();document.getElementById('cpl-role').value=r?.role||'Content Creator';document.getElementById('cpl-account').value=r?.account||'HQ';document.getElementById('cpl-type').value=r?.type||'Video';document.getElementById('cpl-status').value=r?.status||'belum';document.getElementById('cpl-link').value=r?.resultLink||'';document.getElementById('cpl-title').value=r?.title||'';document.getElementById('cpl-note').value=r?.note||'';
  document.getElementById('creative-plan-modal').classList.add('open');
}
document.getElementById('creative-add-plan')?.addEventListener('click',()=>openPlan());
document.getElementById('creative-calendar-grid')?.addEventListener('click',e=>{const d=e.target.closest('[data-cal-date]');if(d)openPlan(null,d.dataset.calDate);});
document.getElementById('creative-plan-form')?.addEventListener('submit',async e=>{e.preventDefault();const d={date:document.getElementById('cpl-date').value,role:document.getElementById('cpl-role').value,account:document.getElementById('cpl-account').value,type:document.getElementById('cpl-type').value,status:document.getElementById('cpl-status').value,resultLink:document.getElementById('cpl-link').value.trim(),title:document.getElementById('cpl-title').value.trim(),note:document.getElementById('cpl-note').value.trim(),updatedAt:firebase.firestore.FieldValue.serverTimestamp()};try{if(creativePlanEditingId)await db.collection('creativePlans').doc(creativePlanEditingId).update(d);else{d.createdAt=firebase.firestore.FieldValue.serverTimestamp();await db.collection('creativePlans').add(d);}toast('Planning disimpan ✓');document.getElementById('creative-plan-modal').classList.remove('open');creativePlanEditingId=null;}catch(err){toast('Gagal simpan planning: '+err.message,true);}});
document.getElementById('creative-plan-body')?.addEventListener('click',async e=>{const ed=e.target.closest('[data-pledit]'),del=e.target.closest('[data-pldelete]');if(ed)openPlan(ed.dataset.pledit);if(del&&confirm('Padam planning ini?'))await db.collection('creativePlans').doc(del.dataset.pldelete).delete();});
...
// ================= V64 CREATIVE ANALYTICS =================
let creativeAnalyticsRows=[],creativeAnalyticsStarted=false,creativeAnalyticsEditId=null,creativeAnalyticsProof='';
function caNum(v){return Number(v)||0}
function caCompress(file){return new Promise((res,rej)=>{const rd=new FileReader();rd.onload=()=>{const im=new Image();im.onload=()=>{let w=im.width,h=im.height,s=Math.min(1,1200/w,1200/h);w=Math.round(w*s);h=Math.round(h*s);const cv=document.createElement('canvas');cv.width=w;cv.height=h;cv.getContext('2d').drawImage(im,0,0,w,h);let out=cv.toDataURL('image/jpeg',.7);if(out.length>850000)out=cv.toDataURL('image/jpeg',.5);res(out)};im.onerror=rej;im.src=rd.result};rd.onerror=rej;rd.readAsDataURL(file)})}
function startCreativeAnalyticsListener(){if(creativeAnalyticsStarted)return;creativeAnalyticsStarted=true;db.collection('creativeAnalytics').onSnapshot(s=>{creativeAnalyticsRows=s.docs.map(d=>({id:d.id,...d.data()}));renderCreativeAnalytics()},e=>toast('Ralat baca Creative Analytics: '+e.message,true))}
function caRows(){const f=document.getElementById('creative-from')?.value||'',t=document.getElementById('creative-to')?.value||'';return creativeAnalyticsRows.filter(x=>(!f||x.date>=f)&&(!t||x.date<=t))}
function renderCreativeAnalytics(){const rows=caRows(),set=(id,v)=>{const e=document.getElementById(id);if(e)e.textContent=v};set('ca-total',rows.length);set('ca-growth','+'+rows.reduce((a,x)=>a+caNum(x.followerGain),0).toLocaleString());set('ca-comments',rows.reduce((a,x)=>a+caNum(x.comments),0).toLocaleString());set('ca-proof',rows.filter(x=>x.proof).length);const latest={};rows.slice().sort((a,b)=>(a.date||'').localeCompare(b.date||'')).forEach(x=>latest[x.account]=x);set('ca-followers',Object.values(latest).reduce((a,x)=>a+caNum(x.followers),0).toLocaleString());const ag=document.getElementById('ca-account-grid');if(ag){const names=['HQ','Solusi','Mamayyuu','Manjaratu V+','YGROW'];ag.innerHTML=names.map(name=>{const ar=rows.filter(x=>x.account===name),l=[...ar].sort((a,b)=>(b.date||'').localeCompare(a.date||''))[0],g=ar.reduce((a,x)=>a+caNum(x.followerGain),0),cm=ar.reduce((a,x)=>a+caNum(x.comments),0),cl=name==='HQ'?'acc-hq':name==='Solusi'?'acc-solusi':name==='Mamayyuu'?'acc-mamayyuu':name==='Manjaratu V+'?'acc-manjaratu':'acc-ygrow';return `<article class="creative-account-card ${cl}"><div class="creative-account-card-top"><div><b>${name}</b><span>${ar.length} update</span></div><strong>${(l?caNum(l.followers):0).toLocaleString()}</strong></div><div class="ca-account-stats"><span>+${g.toLocaleString()} follower</span><span>${cm.toLocaleString()} komen</span></div><small>${l?'Latest '+l.date:'Belum ada data'}</small></article>`}).join('')}
 const b=document.getElementById('ca-body');if(b)b.innerHTML=rows.length?[...rows].sort((a,b)=>(b.date||'').localeCompare(a.date||'')).map(x=>`<tr><td>${x.date||'-'}</td><td><b>${cEsc(x.account||'-')}</b></td><td>${caNum(x.followers).toLocaleString()}</td><td><span class="ca-positive">+${caNum(x.followerGain).toLocaleString()}</span></td><td>${caNum(x.comments).toLocaleString()}</td><td>${x.proof?`<button class="creative-files-btn" data-ca-proof="${x.id}">Lihat Bukti</button>`:'–'}</td><td>${cEsc(x.note||'-')}</td><td><button class="creative-action edit" data-ca-edit="${x.id}">Edit</button><button class="creative-action delete" data-ca-delete="${x.id}">Padam</button></td></tr>`).join(''):'<tr><td colspan="8" class="empty-state">Belum ada update analytics dalam julat tarikh ini.</td></tr>'}
function caOpen(id=null){creativeAnalyticsEditId=id;const x=id?creativeAnalyticsRows.find(a=>a.id===id):null;creativeAnalyticsProof=x?.proof||'';document.getElementById('ca-form-title').textContent=id?'Edit Analytics':'Update Analytics Harian';document.getElementById('ca-date').value=x?.date||new Date().toISOString().slice(0,10);document.getElementById('ca-account').value=x?.account||'HQ';document.getElementById('ca-follower').value=x?.followers??'';document.getElementById('ca-gain').value=x?.followerGain??0;document.getElementById('ca-comment').value=x?.comments??0;document.getElementById('ca-note').value=x?.note||'';document.getElementById('ca-proof-preview').innerHTML=creativeAnalyticsProof?`<img src="${creativeAnalyticsProof}" alt="Bukti">`:'';document.getElementById('creative-analytics-modal').classList.add('open')}
document.getElementById('creative-add-analytics')?.addEventListener('click',()=>caOpen());document.getElementById('ca-proof-file')?.addEventListener('change',async e=>{const f=e.target.files?.[0];if(!f)return;creativeAnalyticsProof=await caCompress(f);document.getElementById('ca-proof-preview').innerHTML=`<img src="${creativeAnalyticsProof}" alt="Bukti">`});document.getElementById('creative-analytics-form')?.addEventListener('submit',async e=>{e.preventDefault();const d={date:document.getElementById('ca-date').value,account:document.getElementById('ca-account').value,followers:caNum(document.getElementById('ca-follower').value),followerGain:caNum(document.getElementById('ca-gain').value),comments:caNum(document.getElementById('ca-comment').value),note:document.getElementById('ca-note').value.trim(),proof:creativeAnalyticsProof,updatedAt:firebase.firestore.FieldValue.serverTimestamp()};try{if(creativeAnalyticsEditId)await db.collection('creativeAnalytics').doc(creativeAnalyticsEditId).update(d);else{d.createdAt=firebase.firestore.FieldValue.serverTimestamp();await db.collection('creativeAnalytics').add(d)}document.getElementById('creative-analytics-modal').classList.remove('open');toast('Creative Analytics disimpan ✓')}catch(err){toast('Gagal simpan analytics: '+err.message,true)}});document.getElementById('ca-body')?.addEventListener('click',async e=>{const eb=e.target.closest('[data-ca-edit]'),dbt=e.target.closest('[data-ca-delete]'),pb=e.target.closest('[data-ca-proof]');if(eb)caOpen(eb.dataset.caEdit);if(dbt&&confirm('Padam rekod analytics ini?'))await db.collection('creativeAnalytics').doc(dbt.dataset.caDelete).delete();if(pb){const x=creativeAnalyticsRows.find(a=>a.id===pb.dataset.caProof);if(x?.proof){const w=window.open();w.document.write(`<img src="${x.proof}" style="max-width:100%;height:auto">`)}}});
...
function activateCreativeSection(tab){
  ...
  if(tab==='analytics'){startCreativeAnalyticsListener();renderCreativeAnalytics();}
}
function renderCreativeDashboard(){
  ...
  if(ag && typeof CREATIVE_ACCOUNT_TARGETS!=='undefined'){
    ag.innerHTML=Object.entries(CREATIVE_ACCOUNT_TARGETS).map(([name,cfg])=>{
      ...
      const accountCls=name==="HQ"?"acc-hq":name==="Solusi"?"acc-solusi":name==="Mamayyuu"?"acc-mamayyuu":name==="Manjaratu V+"?"acc-manjaratu":"acc-ygrow";return `<article class="creative-account-card ${accountCls}"><div class="creative-account-card-top"><div><b>${name}</b><span>${count}/${tar} posted</span></div><strong>${p}%</strong></div><div class="creative-progress"><i style="width:${p}%"></i></div><small>Target ${cfg.total} content / hari</small></article>`;
    }).join('');
  }
