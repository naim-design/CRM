...
document.getElementById('creative-poster-form')?.addEventListener('submit',async e=>{e.preventDefault();const d={kind:'poster',date:document.getElementById('cp-date').value,platform:document.getElementById('cp-platform').value,status:document.getElementById('cp-status').value,title:document.getElementById('cp-title').value.trim(),note:document.getElementById('cp-note').value.trim(),mediaData:creativePosterImages[0]||creativePosterImageData||'',mediaFiles:creativePosterImages,updatedAt:firebase.firestore.FieldValue.serverTimestamp()};try{creativeEditingId?await db.collection('creativeTracker').doc(creativeEditingId).update(d):(d.createdAt=firebase.firestore.FieldValue.serverTimestamp(),await db.collection('creativeTracker').add(d));toast('Poster disimpan ✓');cClose('creative-poster-modal');creativePosterImageData='';creativePosterImages=[];}catch(err){toast('Gagal simpan: '+err.message,true);}});
...
document.getElementById('creative-poster-body')?.addEventListener('click',async e=>{const ed=e.target.closest('[data-pedit]'),del=e.target.closest('[data-cdelete]'),files=e.target.closest('[data-files]');if(ed)cOpen('poster',ed.dataset.pedit);if(files){const r=creativeRows.find(x=>x.id===files.dataset.files);const arr=r?.mediaFiles?.length?r.mediaFiles:(r?.mediaData?[r.mediaData]:[]);if(arr.length){const ov=document.createElement('div');ov.className='creative-lightbox';ov.innerHTML=`<div class="creative-lightbox-card"><button class="creative-lightbox-close">×</button><div class="creative-lightbox-grid">${arr.map((s,i)=>`<a href="${s}" target="_blank"><img src="${s}" alt="file ${i+1}"></a>`).join('')}</div></div>`;document.body.appendChild(ov);ov.addEventListener('click',x=>{if(x.target===ov||x.target.closest('.creative-lightbox-close'))ov.remove();});}}if(del&&confirm('Padam rekod ini?'))await db.collection('creativeTracker').doc(del.dataset.cdelete).delete();});
