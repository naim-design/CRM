...
document.querySelectorAll('[data-creative-sidebar-tab]').forEach(btn=>{
  btn.addEventListener('click',()=>{
    ...
    const hiddenTab=document.querySelector(`[data-creative-tab="${tab}"]`);
    if(hiddenTab) hiddenTab.click();
    else{
      creativeActiveTab=tab;
      document.querySelectorAll('.creative-tab-pane').forEach(p=>p.classList.toggle('active',p.id===`creative-tab-${tab}`));
    }
    syncCreativeSidebarSubnav(tab);
  });
});
